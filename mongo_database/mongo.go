...
// IsInit checking if database is already connected
...
func InitConnect(dbKey string, option *UrlOption) (err error) {
	if IsInit(dbKey) {
		log.Printf("database has been connected to key[%s]", dbKey)
		return
	}
	var url string
	var dbName string
	if option == nil {
		...
		if !ok {
			sysLogger.Config.Errorf("mongo configuration file does not exist,Key[%s]", dbKey)
			os.Exit(0)
		}
	} else {
		url = option.Url
	...
	log.Println("starting to connect to database", url)
	ctx, _ := context.WithTimeout(context.TODO(), time.Second*2)
	client, err := mongo.Connect(ctx, options.Client().ApplyURI(url))
	if err != nil {
		log.Println("	an error occurred while connecting", err)
		return
	}
	dbSession := client.Database(dbName)
	log.Println("selecting database", dbName)
	...
	log.Println("database selection complete", dbName)
	return
}
